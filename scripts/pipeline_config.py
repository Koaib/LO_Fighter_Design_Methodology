...
main.py (the pipeline itself) and extract_params.py (parameter-dump /
classification-template generator) both import IMPORT_FILE/GEOMETRY_DIR/
REF_WING_NAME from here instead of each hardcoding their own copy —
which is what previously let extract_params.py silently point at a
different geometry file than main.py was actually running.

Edit IMPORT_FILE here — not in main.py, not in extract_params.py — and
